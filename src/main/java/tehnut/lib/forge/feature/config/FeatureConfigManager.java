package tehnut.lib.forge.feature.config;
            ...
            if (field.isAnnotationPresent(Feature.Config.class) && Modifier.isStatic(field.getModifiers()) && !Modifier.isFinal(field.getModifiers())) {
                ...
                if (typeAdapter != null) {
                    try {
                        ...
                        LendingLibrary.LOGGER.error("Failed to handle config value for {}. {}", field.getName(), e);
                        e.printStackTrace();
                    }
                    ...
                    LendingLibrary.LOGGER.error("Failed to handle config value for {}. Unknown value type.", field.getName());
                }
            }
        }
